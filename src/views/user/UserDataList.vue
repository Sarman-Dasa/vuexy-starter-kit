<template>
  ...
  export default {
    ...
    methods: {
      ...
      //fill user table 
      async fillUserList() {
        let response = await this.getHTTPPostResponse("list",{},false);
      ...
      async changeUserRole(id) {
        ...
        let response = await this.getHTTPPutResponse(`update-role/${this.userId}`,input,true);
        if(response && response.data) {
            this.$refs['user-role'].hide();
            this.fillUserList();
        }
      },
      ...
      async updateTodoStatus(id,status) {
        ...
        let response = await this.getHTTPPutResponse(`update-status/${id}`,input,true);
        if(response && response.data) {
            this.fillUserList();
        }
      }
